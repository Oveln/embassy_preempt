...
- **OSTaskCreate/OSTaskCreateExt** - 任务创建
- **OSTimeDly** - 任务延迟  
- **OSTimeDlyHMSM** - 按时分秒延迟
- **OSSemCreate/OSSemPend/OSSemPost** - 信号量操作
...
任务可以通过`wait_for_ipi().await`异步等待来自StarryOS的RPC调用或Notification。
...
| **浮点单元** | 无 | 双精度浮点 |
| **数据缓存** | 无 | 完整L1+L2 |
| **上下文切换** | 1.25~2.48 μs | 平均1μs，最大2μs |
