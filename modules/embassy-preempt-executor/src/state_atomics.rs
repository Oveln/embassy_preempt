...
impl State {
    ...
    pub fn run_enqueue(&self) -> bool {
        #[cfg(target_arch = "arm")]
        {
            // ARM-specific optimized implementation using LDREX/STREX
            unsafe {
                loop {
                    let state: u32;
        ...
        {
            // Generic implementation using standard atomic operations
            compiler_fence(Ordering::Release);

            let current_state = self.as_u32().load(Ordering::Relaxed);

            // Check if already run-queued or not spawned
            if (current_state & STATE_RUN_QUEUED != 0) || (current_state & STATE_SPAWNED == 0) {
                compiler_fence(Ordering::Acquire);
                return false;
            }

            // Try to set the run-queued flag using standard atomic compare_exchange loop
            loop {
                let current_state = self.as_u32().load(Ordering::Relaxed);

                // Double-check condition inside the loop
                if (current_state & STATE_RUN_QUEUED != 0) || (current_state & STATE_SPAWNED == 0) {
                    compiler_fence(Ordering::Acquire);
                    return false;
                }

                let new_state = current_state | STATE_RUN_QUEUED;

                match self.as_u32().compare_exchange(
                    current_state,
                    new_state,
                    Ordering::Relaxed,
                    Ordering::Relaxed,
